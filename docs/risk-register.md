...
| --- | --- | --- | --- | --- | --- | --- |
| RSK-001 | Editing posted financial data may violate business, audit, or legal rules. | High | Medium | Critical | Require approval when configured, reason, ticket, policy allow-list, sandbox validation, and clear posted-data control guidance. | Open |
...
| RSK-006 | The BC platform may block writes to some tables or field types. | Medium | High | High | Sandbox validation before implementation claims support. | Open |
...
| RSK-018 | Batch entry may increase the blast radius of a mistaken correction request. | High | Medium | High | Batch entry only creates standard request lines; execution remains gated by policy, preview, approval, audit, and rollback controls. | Open |
...
| RSK-020 | Operation-type staging may make rename, delete, or insert support look executable before readiness allows it. | High | Medium | High | Keep execution and rollback blocked, label operation type as staging only in foundation docs/UI, and require sandbox mutation and rollback validation before enabling operation-specific writes. | Open |
