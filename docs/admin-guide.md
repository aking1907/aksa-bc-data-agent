...
14. Confirm `Record ID` is read-only, the `Select Record` line action opens target record lookup for existing-record operation types, selecting a row fills the canonical identity, and `Insert` keeps `Record ID` empty.
...
19. Run `Execute` only on artificial sandbox data for supported grouped `Update` lines and confirm unsupported operation types are audited as blocked.
