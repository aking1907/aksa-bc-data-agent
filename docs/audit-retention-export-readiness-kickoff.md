...
Phase 8 is not just reporting. It can expose sensitive values through exports and can delete app-owned operation records through retention cleanup. The local implementation therefore keeps export filtered and redacted, protects active/incomplete records during cleanup, and still requires sandbox validation before release.

## Current Start Decision

Audit, retention, and export local implementation is complete for the controlled Phase 8 slice.

Implemented code includes `BCDA Audit Export Mgt.`, filtered export from `BCDA Audit Entries`, and retention cleanup from `BCDA Setup`.
...
| 3 | Redaction rules are implemented | Export omits target record identity text and rollback snapshot payloads; sanitized errors only | Local export implemented |
| 4 | Export format is confirmed | CSV layout, file naming, required filters, and value redaction rules are documented | Local export implemented |
...
- Export only app-owned audit metadata.
- Redact or omit target values and rollback snapshot payloads by default.
- Require explicit filters for request, date range, company, operation, or result.
...
- `docs/test-plan.md` includes audit export redaction, filter coverage, non-`SUPER` blocking, retention cleanup success, active-request protection, cleanup failure, and upgrade readability scenarios.
