...
| AC-006 | A normal update or supported delete is allowed by policy and the configured workflow does not require separate approval or paper confirmation | A `SUPER` user executes it after required runtime controls pass | The request's supported target field changes and record deletes are applied as one transaction, mandatory audit metadata is written, and retained value refs are linked only for rollback-capable update lines when rollback snapshots are enabled. |
...
| AC-022 | Rollback snapshot logging is enabled by setup or policy | A `SUPER` user executes a rollback-capable update correction | Before-image snapshots are stored with retention category and expiration date. |
...
| AC-033 | A supported record-level `Delete` line is allowed by policy | A `SUPER` user executes it after required runtime controls pass | The selected target record is deleted as part of the request transaction, mandatory audit metadata is written, rollback snapshots are not created for the `Delete` line, and rollback availability states that delete rollback is unavailable. |
