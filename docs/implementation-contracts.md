...
This file records implementation-level commitments. Phase 2-8 foundation, preview, grouped update execution, supported primary-key rename execution, supported record-level delete execution, supported grouped insert execution, supported update rollback, correction request Excel export/import, filtered audit metadata export, and governed retention cleanup objects now exist in AL. Local code development is open under standing authorization; non-update rollback, conflict override, unredacted export, snapshot payload export, and external APIs must stay runtime-gated until their operation-specific controls and validation evidence exist.
...
| Services | BCDA Correction Orchestrator, BCDA Policy Guard, BCDA Audit Writer, BCDA Current Value Mgt., BCDA Rollback Service, BCDA Audit Export Mgt., BCDA Request Excel Export Mgt., BCDA Value Serializer, BCDA Metadata Explorer, BCDA Validation Runner | Coordinate behavior without putting business logic in pages. |
...
| Codeunits | `BCDA Access Mgt.`, `BCDA Setup Mgt.`, `BCDA Policy Guard`, `BCDA Audit Writer`, `BCDA Current Value Mgt.`, `BCDA Value Serializer`, `BCDA Correction Orchestrator`, `BCDA Batch Line Mgt.`, `BCDA Retention Manager`, `BCDA Metadata Explorer`, `BCDA Rollback Service`, `BCDA Audit Export Mgt.`, `BCDA Request Excel Export Mgt.` |
...
| ExportCorrectionRequestWorkbook | Require `SUPER`, require setup `Export Enabled`, export only staged correction request line data to an Excel workbook with one worksheet per target table, preserve the preview-matrix-style current/new layout, write request-export audit evidence, and never write target records. |
| ImportCorrectionRequestWorkbook | Require `SUPER`, require saved correction request status `Open`, require explicit user confirmation before deleting existing lines, parse all workbook sheets into temporary correction lines, validate request/table/field/record identity/insert group/proposed-value shape before replacement, replace only the request's stored correction lines, write request-import audit evidence, and never write target records. |
