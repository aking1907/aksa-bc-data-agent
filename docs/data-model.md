...
| BCDA Correction Line | One staged correction operation or operation field value. | Request id, line no., correction type, table id, target RecordId when applicable, insert group no. when inserting, display key, field id when applicable, proposed new value when applicable, optional old/new value refs, rollback snapshot mode, snapshot expiration date, validation mode, line status, sanitized error. |
...
- Correction line, batch buffer, and audit schema now use canonical `RecordId` storage where an existing target record is applicable. `Insert` correction lines deliberately keep `RecordId` empty while staged; app-owned `Insert Group No.` groups insert lines by request/table/insert-group, creates one record per group, and stores the created `RecordId` after success for audit/review. The foundation lookup uses a temporary display key; persistent display-key storage can be added before richer target matrix preview is released.
