...
| REQ-032 | The app must classify correction lines by operation type (`Update`, `Rename`, `Delete`, `Insert`) so execution and rollback can group intended record changes by correction type and canonical target identity when applicable; `Rename` lines must use an existing target `RecordId`, stage primary-key fields only, and store the renamed `RecordId` after successful execution; `Insert` lines must not require a target `RecordId` while staged, must create one record per request/table insert group until a richer insert-group identity exists, and may store the created `RecordId` after successful execution for audit/review. | AC-030, AC-034, AC-035 | 2-5 |
