...
Current phase: Phase 8 Audit, Retention, And Export local implementation is complete for supported grouped `Update` corrections, supported `Rename` primary-key execution with rollback-unavailable status, supported record-level `Delete` execution with rollback-unavailable status, supported grouped `Insert` execution with rollback-unavailable status, request-level rollback staging from completed `Update` requests, correction request Excel export/import, filtered audit metadata export, and governed cleanup of expired BCDA-owned operation records. Phase 6 standard `Update`, supported `Rename`, supported `Delete`, and supported `Insert` execution are governed by runtime controls rather than paper-confirmation gates: approval and external ticket/reference evidence are required only when setup or policy requires them. Rename rollback, delete rollback, insert rollback, broader operation-aware non-update rollback, conflict override, unfiltered export, unredacted export, snapshot payload export, and external APIs may be developed locally and remain runtime-gated until their controls and validation evidence exist.
...
| REQ-007 | The app must execute only workflow-eligible and policy-allowed field-level data changes, supported primary-key renames, supported record-level deletes, or supported grouped inserts. A correction request is an all-or-nothing transaction: validation must cover the whole request before mutation, and any runtime mutation error must roll back the request's target writes. Supported execution groups must process in the fixed type sequence `Update`, `Rename`, `Delete`, `Insert`, so same-target update lines run before rename lines that change the record identity. Approval is required only when setup or policy requires approval; standard allowed updates, supported renames, supported deletes, and supported inserts may execute without separate paper confirmation when other runtime controls pass. | AC-006, AC-007, AC-033, AC-034, AC-035 | 4 |
...
| REQ-012 | The app must block destructive operations that are outside implemented controls, including deleting records outside the supported `Delete` workflow, inserting records outside the supported grouped `Insert` workflow, renaming primary keys outside the supported `Rename` workflow, and editing blocked fields. | AC-012, AC-033, AC-034, AC-035 | 4 |
...
| REQ-030 | The app must let `SUPER` users create multiple same-table correction lines through a RecordId-backed batch entry experience that supports simple and composite primary-key target selection, insert grouping, and transformation into the standard correction line structure without changing target data. | AC-028 | 3-4 |
...
| REQ-032 | The app must classify correction lines by operation type (`Update`, `Rename`, `Delete`, `Insert`) so users can understand the target shape before execution and so execution/rollback can group intended record changes by correction type and canonical target identity when applicable; `Rename` lines must use an existing target `RecordId`, stage primary-key fields only, and store the renamed `RecordId` after successful execution; `Insert` lines must not require a target `RecordId` while staged, must use an app-owned `Insert Group No.` to create one record per request/table/insert-group, and may store the created `RecordId` after successful execution for audit/review. | AC-030, AC-034, AC-035 | 2-5 |
| REQ-035 | The app must let a `SUPER` user export a saved correction request to an Excel workbook only when setup export is enabled. The workbook must use one worksheet per target table caption, show request/table identity on the first row, leave the second row blank, and start a preview-matrix-style table on the third row with modification type, target identity or insert group, and the requested field columns. The export must not mutate target data and must write audit evidence. | AC-036 | 3-6 |
| REQ-036 | The app must let a `SUPER` user import correction request lines from an Excel workbook only while the correction request is `Open`. Before import, the user must confirm existing request lines will be deleted and recreated. The import must validate workbook request/table identity, field headers, target record identities for existing-record operations, insert group numbers for insert operations, and proposed values into temporary correction lines before replacing stored request lines. The import must not mutate target data and must write audit evidence. | AC-037 | 3-6 |
